...
for eidx in range(1, N_energies):
    ...
    QHO_lim_idx = min(
        np.where(U_QHO[int((N_phis+1)/2):N_phis] > delta_E_QHO)[0])
    trans_lim_idx = min(
        np.where(U_transmon[int((N_phis+1)/2):N_phis] > delta_E_transmon)[0])
...
axes.set_xlabel('Phase $\phi$', fontsize=10)
axes.set_ylabel('Energy Levels / $\hbar\omega$', fontsize=10)
axes.set_ylim(-0.2, 5)
